...
#include "common/yolo_world_text.hpp"
...
int main(int argc, char** argv) {
  ...
  auto input_tokens = vsx::ReadNpzFile(args.get<std::string>("test_npz_file"));
  auto warmup_times = args.get<uint32_t>("warmup_times");

  std::vector<std::shared_ptr<vsx::YoloWorldText>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::YoloWorldText>(
  ...
  vsx::ModelProfiler<vsx::YoloWorldText> profiler(config, models, warmup_times);
