...
#include "common/model_profiler_async.hpp"
...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<uint32_t>("warmup_times", '\0', "warmup times before profiling",
                     false, 10);
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::ClassifierAsync>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::ClassifierAsync>(
  ...
  vsx::ModelProfilerAsync<vsx::ClassifierAsync> profiler(config, models,
                                                         warmup_times);
