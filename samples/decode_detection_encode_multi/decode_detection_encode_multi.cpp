...
int Process(const cmdline::parser& args, std::string input_uri,
  ...
  for (uint32_t inn = 0; inn < loop; inn++) {
    ...
    // create video_encoder
    auto disable_encoder = args.get<uint32_t>("disable_encoder");
    std::thread encoder_consumer;
    std::unique_ptr<vsx::VideoEncoder> video_encoder;
    if (!disable_encoder) {
      ...
      conf.frame_rate_numerator = frame_rate;
      conf.frame_rate_denominator = 1;
      video_encoder = std::make_unique<vsx::VideoEncoder>(
          codec_type, frame_width, frame_height, conf);
      // create thread to write encoded frame data
      encoder_consumer = std::thread([&]() {
        // create encoder result file
        auto output_path = args.get<std::string>("output_path");
        auto outfile_name =
            output_path + "/channel_" + std::to_string(index) + ".ts";
        std::ofstream encoded_file(outfile_name, std::ios::binary);
        if (!encoded_file.is_open()) {
          std::cout << "Error, Failed to open encode result file: "
    ...
    if (!disable_encoder) {
      std::cout << "Stop video encoder\n";
      video_encoder->StopSendImage();
      encoder_consumer.join();
    }
  }
  return 0;
}
