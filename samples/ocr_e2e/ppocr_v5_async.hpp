...
class PPOCR_v5_Async {
 public:
  PPOCR_v5_Async(
    ...
    doc_ori_cls_thread_ =
        std::thread(&PPOCR_v5_Async::DocImgOrientClsThread, this);
    det_thread_ = std::thread(&PPOCR_v5_Async::DetectThread, this);
    det_post_thread_ = std::thread(&PPOCR_v5_Async::DetectPostThread, this);
    text_ori_cls_thread_ =
        std::thread(&PPOCR_v5_Async::TextLineOrientClsThread, this);
    rec_thread_ = std::thread(&PPOCR_v5_Async::RecognizeThread, this);
  }
  ~PPOCR_v5_Async() {
              ...
              for (int i = 0; i < obj_count; i++) {
                ...
                if (det_box_type_ == "quad") {
                  crop_images.emplace_back(
                      GetRotateCropImage(vsx_image, src_points));
                } else {
                  crop_images.emplace_back(
