...
int main(int argc, char* argv[]) {
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    ops.push_back(std::make_shared<vsx::NormaTensor3ChOp>("norma_tensor_3ch_op",
                                                          elf_file, device_id));
  }
  ...
  vsx::ModelProfiler<vsx::NormaTensor3ChOp> profiler(config, ops);
