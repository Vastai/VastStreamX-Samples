...
int main(int argc, char* argv[]) {
  ...
  uint32_t queue_size = 0;  // queue_size不为0，会出错
  auto percentiles = vsx::ParseVecUint(args.get<std::string>("percentiles"));
  const auto shape = vsx::ParseShape(args.get<std::string>("shape"));

  std::vector<std::shared_ptr<vsx::BrightnessOp>> ops;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    ops.push_back(std::make_shared<vsx::BrightnessOp>(
        "img_brightness_adjust", elf_file, device_id, scale));
  }

  vsx::ProfilerConfig config = {instance,    iterations,  batch_size,
                                vsx::kUint8, device_ids,  contexts,
                                {shape},     percentiles, queue_size};
  vsx::ModelProfiler<vsx::BrightnessOp> profiler(config, ops);
