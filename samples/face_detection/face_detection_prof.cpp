...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>("percentiles", '\0', "percentiles of latency", false,
                        "[50, 90, 95, 99]");
  args.add<float>("threshold", '\0', "threshold for detection", false, 0.01);
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::FaceDetector>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::FaceDetector>(
  ...
  vsx::ModelProfiler<vsx::FaceDetector> profiler(config, models, warmup_times);
