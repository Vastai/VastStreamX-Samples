...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>("vdsp_params", '\0', "vdsp preprocess parameter file",
                        false, "../data/configs/resnet_rgbplanar.json");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::Resnet>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::Resnet>(
  ...
  vsx::ModelProfiler<vsx::Resnet> profiler(config, models, warmup_times);
