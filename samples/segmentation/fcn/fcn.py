...
def argument_parser():
    ...
    parser.add_argument(
        "-m",
        "--model_prefix",
        default="/opt/vastai/vaststreamx/data/models/fcn16s_vgg16-int8-kl_divergence-1_3_320_320-vacc/mod",
    ...
    parser.add_argument(
        "--vdsp_params",
        default="../data/configs/fcn_bgr888.json",
    ...
    parser.add_argument(
        "--output_file",
        default="./fcn_result.jpg",
        help="output file",
    )
    parser.add_argument(
        "--dataset_filelist",
        default="",
        help="dataset image file list",
    )
    parser.add_argument(
        "--dataset_root",
        default="",
...
if __name__ == "__main__":
    ...
    if args.dataset_filelist == "":
        ...
        for i, filename in enumerate(filelist):
            filename = os.path.join(args.dataset_root, filename.strip(" ").strip("\n"))
            print(filename)
            cv_image = cv2.imread(filename)
            assert cv_image is not None, f"Failed to read image:{filename}"
