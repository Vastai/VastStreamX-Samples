...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>("model_prefix", 'm',
                        ...
                        "fcn16s_vgg16-int8-kl_divergence-1_3_320_320-vacc/mod");
  ...
  args.add<std::string>("vdsp_params", '\0', "vdsp preprocess parameter file",
                        false, "../data/configs/fcn_bgr888.json");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::Segmentator>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::Segmentator>(
  ...
  vsx::ModelProfiler<vsx::Segmentator> profiler(config, models, warmup_times);
