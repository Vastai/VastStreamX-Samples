...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::Mask2Former>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::Mask2Former>(
        model_prefix, vdsp_params, batch_size, device_id, 0.5, hw_config));
  }
  vsx::TShape shape;
  ...
  vsx::ModelProfiler<vsx::Mask2Former> profiler(config, models, warmup_times);
