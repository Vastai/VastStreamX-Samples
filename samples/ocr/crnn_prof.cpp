...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::CRNN>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::CRNN>(model_prefix, vdsp_params,
                                                 batch_size, device_id,
                                                 label_file, hw_config));
  }
  vsx::TShape shape;
  ...
  vsx::ModelProfiler<vsx::CRNN> profiler(config, models, warmup_times);
