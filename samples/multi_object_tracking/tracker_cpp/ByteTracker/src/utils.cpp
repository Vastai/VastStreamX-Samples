...
void BYTETracker::remove_duplicate_stracks(vector<STrack> &resa,
  ...
  for (size_t i = 0; i < stracksa.size(); i++) {
    vector<size_t>::iterator iter = find(dupa.begin(), dupa.end(), i);
    if (iter == dupa.end()) {
  ...
  for (size_t i = 0; i < stracksb.size(); i++) {
    vector<size_t>::iterator iter = find(dupb.begin(), dupb.end(), i);
    if (iter == dupb.end()) {
...
double BYTETracker::lapjv(const vector<vector<float> > &cost,
  ...
  for (size_t i = 0; i < n; i++) cost_ptr[i] = new double[sizeof(double) * n];
