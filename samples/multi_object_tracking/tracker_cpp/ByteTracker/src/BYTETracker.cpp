...
BYTETracker::BYTETracker(int frame_rate, int track_buffer, float track_thre) {
  track_thresh = track_thre;
  high_thresh = track_thre + 0.1;
  match_thresh = 0.9;

  frame_id = 0;
  max_time_lost = int(frame_rate / 30.0 * track_buffer + 0.5);
}

BYTETracker::~BYTETracker() {}

void print_tracks(const std::string &name, const std::vector<STrack> &stracks) {
  std::cout << name << ":\n";
  for (const auto &s : stracks) {
    std::cout << s.track_id << ", [ ";
    for (auto num : s.tlwh) {
      std::cout << num << " ";
    }
...
void print_tracks_ptr(const std::string &name,
...
vector<STrack> BYTETracker::update(const vector<Object> &objects) {
  ...
  // std::cout << " this->high_thresh: " << this->high_thresh << std::endl;
  for (size_t i = 0; i < u_detection.size(); i++) {
...
vector<vector<float>> BYTETracker::fuse_score(
    const vector<vector<float>> &dists, const vector<STrack> &detections) {
  if (dists.size() == 0) {
    return dists;
  }

  vector<vector<float>> iou_sim;
  iou_sim.reserve(dists.size());
  for (const auto &dist : dists) {
    vector<float> sim;
    sim.reserve(dist.size());
    for (auto d : dist) {
      sim.push_back(1.0f - d);
    }
    iou_sim.push_back(sim);
  }
  vector<float> det_scores;
  det_scores.reserve(detections.size());
  for (const auto &det : detections) {
    det_scores.push_back(det.score);
  }

  vector<vector<float>> fuse_cost;
  fuse_cost.reserve(dists.size());
  for (auto &sim : iou_sim) {
    vector<float> costs;
    for (size_t i = 0; i < sim.size(); i++) {
      float cost = 1.0 - sim[i] * det_scores[i];
      costs.push_back(cost);
    }
    fuse_cost.push_back(costs);
  }
  return fuse_cost;
}