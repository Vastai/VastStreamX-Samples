...
int main(int argc, char** argv) {
  ...
  auto norm_elf = args.get<std::string>("norm_elf");
  auto space2depth_elf = args.get<std::string>("space2depth_elf");
  ...
  std::vector<std::shared_ptr<vsx::ClipImage>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::ClipImage>(
        model_prefix, norm_elf, space2depth_elf, batch_size, device_id,
        hw_config));
  }
  vsx::TShape shape;
  ...
  vsx::ModelProfiler<vsx::ClipImage> profiler(config, models, warmup_times);
