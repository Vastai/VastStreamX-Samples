...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>("model_prefix", 'm',
                        ...
                        "rtdetr-fp16-none-1_3_640_640-vacc/mod");
  ...
  args.add<std::string>("vdsp_params", '\0', "vdsp preprocess parameter file",
                        false, "./data/configs/rtdetr_bgr888.json");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::RtDetrModel>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::RtDetrModel>(
        model_prefix, vdsp_params, batch_size, device_id, threshold,
        hw_config));
  }
  vsx::TShape shape;
  ...
  vsx::ModelProfiler<vsx::RtDetrModel> profiler(config, models, warmup_times);
