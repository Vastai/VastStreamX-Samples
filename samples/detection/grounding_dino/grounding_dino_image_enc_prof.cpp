...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>("vdsp_params", '\0', "vdsp preprocess parameter file",
                        false, "./data/configs/groundingdino_bgr888.json");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::GroundingDinoImage>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::GroundingDinoImage>(
  ...
  vsx::ModelProfiler<vsx::GroundingDinoImage> profiler(config, models);
