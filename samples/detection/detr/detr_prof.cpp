...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::DetrModel>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::DetrModel>(model_prefix, vdsp_params,
  ...
  vsx::ModelProfiler<vsx::DetrModel> profiler(config, models, warmup_times);
