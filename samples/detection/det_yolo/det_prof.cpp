...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>("model_prefix", 'm',
                        ...
                        "yolov5m-int8-max-1_3_640_640-vacc-pipeline/mod");
...
int main(int argc, char** argv) {
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::Detector>(model_prefix, vdsp_params,
  ...
  vsx::ModelProfiler<vsx::Detector> profiler(config, models, warmup_times);
