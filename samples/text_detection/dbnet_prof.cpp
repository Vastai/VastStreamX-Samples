...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::DBnetDetector>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::DBnetDetector>(
        model_prefix, vdsp_params, elf_file, batch_size, device_id, hw_config));
    // models.back()->SetOpFile(elf_file);
  }
  vsx::TShape shape;
  ...
  vsx::ModelProfiler<vsx::DBnetDetector> profiler(config, models, warmup_times);
