...
char cnpy::map_type(int vast_type) {
  if (vast_type == vsx::kFloat16) return 'f';
  if (vast_type == vsx::kFloat32) return 'f';

  if (vast_type == vsx::kUint8) return 'u';
  if (vast_type == vsx::kUint16) return 'u';
  if (vast_type == vsx::kUint32) return 'u';

  if (vast_type == vsx::kInt8) return 'i';
  if (vast_type == vsx::kInt16) return 'i';
  if (vast_type == vsx::kInt32) return 'i';

  return '?';
}

int cnpy::map_type(const std::string& dtype) {
  if (dtype == "uint8") return vsx::kUint8;
  if (dtype == "int32") return vsx::kInt32;
  if (dtype == "uint16") return vsx::kUint16;
  if (dtype == "uint32") return vsx::kUint32;
  if (dtype == "int8") return vsx::kInt8;
  if (dtype == "int16") return vsx::kInt16;
  if (dtype == "float16") return vsx::kFloat16;
  if (dtype == "float32") return vsx::kFloat32;
  // LOG(FATAL) << "unsupported dtype: " << dtype;
}

int cnpy::map_type(char type, size_t ws) {
  if (type == 'u') {
    if (ws == 1) return vsx::kUint8;
    if (ws == 2) return vsx::kUint16;
    if (ws == 4) return vsx::kUint32;
  }
  if (type == 'i') {
    if (ws == 1) return vsx::kInt8;
    if (ws == 2) return vsx::kInt16;
    if (ws == 4) return vsx::kInt32;
  }
  if (type == 'f') {
    if (ws == 2) return vsx::kFloat16;
    if (ws == 4) return vsx::kFloat32;
  }
  return -1;
}
...
vsx::Tensor load_npy_to_tensor(FILE* fp) {
  ...
  // CHECK(dtype >= 0) << "unsupported data type";
  //  std::cout << "load shape: " << shape << std::endl;
  vsx::Tensor tensor(cvt_shape(shape), vsx::Context::CPU(0), dtype);
  auto nread =
      fread(tensor.MutableData<uint8_t>(), 1, tensor.GetDataBytes(), fp);
  // CHECK(nread == tensor.GetDataBytes())
  //     << "failed fread total " << tensor.GetDataBytes() << " bytes";
...
vsx::Tensor load_npz_array_to_tensor(FILE* fp, uint32_t compr_bytes,
  ...
  // CHECK(dtype >= 0) << "unsupported data type";
  vsx::Tensor tensor(cvt_shape(shape), vsx::Context::CPU(0), dtype);
  size_t offset = uncompr_bytes - tensor.GetDataBytes();
  memcpy(tensor.MutableData<uint8_t>(), &buffer_uncompr[0] + offset,
         tensor.GetDataBytes());
