...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::Classifier>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::Classifier>(
  ...
  vsx::ModelProfiler<vsx::Classifier> profiler(config, models, warmup_times);
