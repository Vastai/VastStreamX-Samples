...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>(
      ...
      "/opt/vastai/vaststreamx/data/models/gpen-int8-mse-1_3_512_512-vacc/mod");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::ModelCV>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::ModelCV>(
  ...
  vsx::ModelProfiler<vsx::ModelCV> profiler(config, models, warmup_times);
