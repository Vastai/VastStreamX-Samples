...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>("model_prefix", 'm',
                        ...
                        "u2net-int8-percentile-1_3_256_256-vacc/mod");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::U2netDetector>> models;
  ...
  for (uint32_t i = 0; i < instance; i++) {
    ...
    models.push_back(std::make_shared<vsx::U2netDetector>(
  ...
  vsx::ModelProfiler<vsx::U2netDetector> profiler(config, models, warmup_times);
