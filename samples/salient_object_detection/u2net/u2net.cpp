...
#include "common/u2net_detector.hpp"
...
cmdline::parser ArgumentParser(int argc, char** argv) {
  ...
  args.add<std::string>("model_prefix", 'm',
                        ...
                        "/u2net-int8-percentile-1_3_320_320-vacc/mod");
  ...
  args.add<std::string>("vdsp_params", '\0', "vdsp preprocess parameter file",
                        false, "../data/configs/u2net_bgr888.json");
...
void WriteToFile(const vsx::Tensor& fp32_tensor,
...
int main(int argc, char** argv) {
  ...
  vsx::U2netDetector model(args.get<std::string>("model_prefix"),
  ...
  if (args.get<std::string>("dataset_filelist").empty()) {
    ...
    auto output = model.Process(vsx_image);
    WriteToFile(output, {0.0f, 1.0f, 255.0f},
    ...
    for (auto file : filelist) {
      ...
      auto output_file = args.get<std::string>("dataset_output_folder") + "/" +
                         p.stem().string() + ".png";
      WriteToFile(output, {0.0f, 1.0f, 255.0f}, output_file);
    }
  }
