...
void InferenceThread(std::shared_ptr<vsx::PPOCR_Async> model,
...
int main(int argc, char** argv) {
  ...
  std::vector<std::shared_ptr<vsx::PPOCR_Async>> models;
  models.reserve(device_ids.size());
  for (auto device_id : device_ids) {
    auto model = std::make_shared<vsx::PPOCR_Async>(
