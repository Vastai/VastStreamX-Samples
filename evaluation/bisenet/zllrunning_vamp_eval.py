...
def vis_parsing_maps(
    ...
    if save_im:
        ...
        cv2.imwrite(
            save_path[:-4] + "_vis.png", vis_im, [int(cv2.IMWRITE_JPEG_QUALITY), 100]
        )
...
if __name__ == "__main__":
    ...
    parse.add_argument(
        "--input_npz_path", type=str, default="npz_datalist.txt", help="npz text file"
    )
    parse.add_argument(
        "--out_npz_dir",
        type=str,
        default="/home/simplew/code/vamc/vamp/0.2.0/npz_output",
    ...
    with open(args.input_npz_path, "r") as f:
        ...
        for i, line in enumerate(tqdm(file_lines, file=sys.stdout)):
            ...
            resize_image = gt_image.resize(args.input_shape, Image.BILINEAR).convert(
                "RGB"
            )
            ...
            heatmap = np.load(output_npz_list[i], allow_pickle=True)["output_0"].astype(
                np.float32
            )
            ...
            if draw:
                vis_parsing_maps(
                    ...
                    stride=1,
                    save_im=True,
                    save_path=os.path.join(args.draw_dir, file_name),
                )
