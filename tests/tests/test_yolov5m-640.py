...
def test_yolov5m_640_cpp(device_id):
    cmd = f"""
...
#################  performance test
@pytest.mark.fast
def test_yolov5m_640_cpp_performance(device_id):
    # 测试最大吞吐
    cmd = f"""
...
############################# dataset test
@pytest.mark.fast
def test_yolov5m_640_cpp_precision(device_id):
    os.system("mkdir -p yolov5m_out")
    cmd = f"""
    ./build/vaststreamx-samples/bin/detection \
    -m /opt/vastai/vaststreamx/data/models/yolov5m-int8-percentile-1_3_640_640-vacc-pipeline/mod \
    --vdsp_params ./data/configs/yolo_div255_bgr888.json \
    ...
    --dataset_output_folder ./yolov5m_out
    """
    run_cmd(cmd,False)
    ...
    os.system("rm -rf yolov5m_out")
# #####################  python test #####################

################ one image  test
@pytest.mark.fast
@pytest.mark.ai_integration
def test_yolov5m_640_py(device_id):
    cmd = f"""
...
#################  performance test
@pytest.mark.fast
def test_yolov5m_640_py_performance(device_id):
    # 最佳性能与时延
    cmd = f"""
...
############################# dataset test
@pytest.mark.fast
def test_yolov5m_640_py_precision(device_id):
    os.system("mkdir -p yolov5m_out")
    cmd = f"""
    python3 ./samples/detection/det_yolo/detection.py \
    -m /opt/vastai/vaststreamx/data/models/yolov5m-int8-percentile-1_3_640_640-vacc-pipeline/mod \
