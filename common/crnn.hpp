...
class CRNN : public ModelCV {
 public:
  CRNN(const std::string& model_prefix, const std::string& vdsp_config,
 ...
 protected:
  std::vector<vsx::Tensor> ProcessImpl(const std::vector<vsx::Image>& images) {
    ...
    std::vector<vsx::Tensor> result_tensors;
    for (size_t t_index = 0; t_index < outputs.size(); ++t_index) {
      auto& outs = outputs[t_index];
      ...
      for (size_t batch = 0; batch < fp32_tensors.size(); batch++) {
        ...
        int32_t C = key_map_.size() + 1;
        // int32_t L = 25;
        size_t L = fp32_tensors[batch].GetSize() / C;
        // LOG(INFO) << "decode input shape: " << L << ", " << C;
        vsx::Tensor output({static_cast<int>(L + 9)}, vsx::Context::CPU(),
                           vsx::kUint8);
        ...
        for (size_t i = 0; i < L; ++i) {
          ...
          if (max_index > 0 && (!(i > 0 && max_index == last_index))) {
            // LOG(INFO) << "get index:" << max_index<< ", key:" <<
            // key_map_[max_index - 1]<< ", score:" << max_score;
        ...
        if (result.size() > 0 && conf.size() > 0) {
          ...
          *score = mean_conf / conf.size();
          ...
          // LOG(INFO) << "decode result: " << result << ", " << *score;

          CHECK(result.length() <= L);
          *len = result.length();
      ...
      result_tensors.emplace_back(res_tensors[0]);
    }
