...
class Dinov2Model {
 public:
  Dinov2Model(
    ...
    // normalize op
    std::vector<uint16_t> mean_v{14275, 14156, 13951};
    std::vector<uint16_t> std_v{13140, 13099, 13107};
 ...
 protected:
  virtual std::vector<vsx::Tensor> ProcessImpl(
    ...
    for (const auto image : images) {
      ...
      auto norm_out = normalize_op_->Process(resize_out);
      auto space_to_depth_out = space2depth_op_->Process(norm_out);
      batch_inputs.push_back({space_to_depth_out});
    }
    auto batch_outs = stream_->RunSync(batch_inputs);
    for (auto &outs : batch_outs) {
      auto out_fp32 = vsx::ConvertTensorFromFp16ToFp32(outs[0].Clone());
