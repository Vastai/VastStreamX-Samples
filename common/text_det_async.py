...
class TextDetectorAsync(ModelCVAsync):
    def __init__(
        self,
        model_prefix,
        vdsp_config,
        ...
        self.min_size_ = 3
        self.size_queue = queue.Queue(50)
    ...
    def process_async_impl(self, inputs):
        img_sizes = []
        for inp in inputs:
            inp_size = edict(width=inp.width, height=inp.height)
            img_sizes.append(inp_size)
        self.size_queue.put(img_sizes)
        return self.stream_.run_async(inputs)

    def get_output(self):
        vsx.set_device(self.device_id_)
        outputs = self.stream_.get_operator_output(self.model_op_)
        input_size = self.size_queue.get()
        return [
            self.post_process(output, input_size[i].width, input_size[i].height)
    ...
    def post_process(self, fp16_tensors, image_width, image_height):
        ...
        boxes = []
        scores = []
        pred = np.array(vsx.as_numpy(fp16_tensors[0]))[0].astype(np.float32)
